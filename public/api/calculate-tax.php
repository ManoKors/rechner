...
function calculateKfzSteuer(string $vehicle_type, int $displacement, int $co2_emission, int $first_registration_year, int $weight): array {
    ...
    // Hubraum- und CO2-Steuer für PKW ab 2021 (gesetzliche Staffelung)
    if ($first_registration_year >= 2021 && in_array($vehicle_type, ['benzin', 'diesel'])) {
        // Hubraumanteil
        ...
        // CO2-Anteil (nur für Emissionen über 95 g/km, gestaffelt)
        $co2 = max(0, $co2_emission - 95);
        if ($co2 > 0) {
            $co2_tax = 0.0;
            $staffeln = [
                [0, 20, 2.00],   // 96-115
                ...
                [60, 20, 2.90],  // 156-175
                [80, 20, 3.40],  // 176-195
                [100, PHP_INT_MAX, 4.00] // ab 196
            ];
            $rest = $co2;
            foreach ($staffeln as [$start, $anzahl, $satz]) {
                $anz = min($rest, $anzahl);
                if ($anz > 0) {
                    $co2_tax += $anz * $satz;
                    $rest -= $anz;
        ...
        // ...bisherige Logik für ältere Fahrzeuge oder andere Typen...
        if ($vehicle_type === 'benzin') {
            $displacement_tax = ceil($displacement / 100) * 2.00;
        } elseif ($vehicle_type === 'diesel') {
            $displacement_tax = ceil($displacement / 100) * 9.50;
        }
        // CO2-Steuer für Fahrzeuge ab 2009 (vereinfachte Annahme, ggf. anpassen)
        if ($first_registration_year >= 2009) {
            $co2_threshold = 95;
            if ($co2_emission > $co2_threshold) {
                $co2_tax = ($co2_emission - $co2_threshold) * 2.00;
            }
        }
    }
